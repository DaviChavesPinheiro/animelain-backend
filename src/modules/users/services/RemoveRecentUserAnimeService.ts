...
@injectable()
export default class RemoveRecentUserAnimeService {
  constructor(
  ...
  public async execute({
    ...
    if (!checkIfRecentUserAnimeAlreadyExist) {
      throw new AppError('This Recent User Anime does not exists');
    }

    const recentUserAnime = await this.recentUsersAnimesRepository.deleteById(
      checkIfRecentUserAnimeAlreadyExist.id,
    );
