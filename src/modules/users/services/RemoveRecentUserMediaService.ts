...
@injectable()
export default class RemoveRecentUserMediaService {
  constructor(
  ...
  public async execute({
    ...
    if (!checkIfRecentUserMediaAlreadyExist) {
      throw new AppError('This Recent User Media does not exists');
    }

    const recentUserMedia = await this.recentUsersMediasRepository.deleteById(
      checkIfRecentUserMediaAlreadyExist.id,
    );
