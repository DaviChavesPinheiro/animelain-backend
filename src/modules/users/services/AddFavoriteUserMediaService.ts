...
@injectable()
export default class AddFavoriteMediaService {
  constructor(
  ...
  public async execute({
    ...
    if (checkIfFavoriteUserMediaAlreadyExist) {
      throw new AppError('This Favorite User Media already exists');
    }

    const favoriteUserMedia = await this.favoriteUsersMediasRepository.create({
      mediaId,
      userId,
    });
