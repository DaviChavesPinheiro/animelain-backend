...
@injectable()
export default class AddMediaMediaCategoryService {
  constructor(
  ...
  public async execute({
    ...
    if (checkIfMediaCategoryAlreadyExist) {
      throw new AppError('This MediaCategory already exists');
    }

    const mediaCategory = await this.mediasCategoriesRepository.create({
      mediaId,
      categoryId,
      score,
    });
